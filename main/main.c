    ...
    if (xQueueReceive(distance_queue, &distance_cm, pdMS_TO_TICKS(1000)) == pdPASS) // distance updated
    {
      ...
      */
      ...
      else
      {
        ...
        otto_reverse(0.57f);             // Move backward at 50% speed to create some distance from the obstacle
        vTaskDelay(pdMS_TO_TICKS(500)); // Short delay to ensure the robot has stopped before executing the maneuver
        ...
        vTaskDelay(pdMS_TO_TICKS(950)); // Turn for 950ms which is an almost perfect 90° angle for wonky the otto (adjust as needed for a 180 degree turn)
        ...
        {
          if (right_distance > 10.0f)
          {                      // Clear path after turn: Move forward at the defined speed
            otto_forward(speed); // Move forward at the defined speed
            vTaskDelay(pdMS_TO_TICKS(350)); // Short delay to ensure the robot has started moving before checking distance again
            continue;            // Exit the loop to continue normal operation
          }
        }

        otto_turn_left(speed);          // If the path is still blocked after the turn, try turning left instead to attempt to navigate around the obstacle
        vTaskDelay(pdMS_TO_TICKS(950)); // Turn to face the original direction (adjust as needed for a 90 degree turn)
        otto_stop();                    // Stop after turning to ensure a more accurate forward movement
        ...
        otto_turn_right(speed);         // If both turns fail to clear the path, try turning right again to attempt to navigate around the obstacle
        vTaskDelay(pdMS_TO_TICKS(1900)); // Turn to face the original direction (adjust as needed for a 90 degree turn)
        otto_stop();                    // Stop after turning to ensure a more accurate forward movement

        otto_reverse(0.5f);             // Move backward at 50% speed to create some distance from the obstacle
        vTaskDelay(pdMS_TO_TICKS(300)); // Short delay to ensure the robot has stopped before executing the maneuver
        otto_stop();                    // Stop
